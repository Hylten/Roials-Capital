import React from 'react';

interface CapabilitiesProps {
  onEquityClick: () => void;
  onCreditClick: () => void;
  onCapOrigClick: () => void;
  onDealOrigClick: () => void;
}

const capabilities = [
  {
    title: 'PRINCIPAL-LED CAPITAL',
    description: 'Bespoke capital formation for established fund managers. Direct LP dialogue. Controlled counterparty progression.',
    icon: (
      <svg className="w-8 h-8 text-oldgold" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
      </svg>
    )
  },
  {
    title: 'SPECIAL SITUATIONS',
    description: 'Asset-backed liquidity structures and non-dilutive capital against operating assets and balance-sheet collateral.',
    icon: (
      <svg className="w-8 h-8 text-oldgold" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
      </svg>
    )
  },
  {
    title: 'OFF-MARKET M&A',
    description: 'Proprietary target acquisition for buyout mandates across the EMEA mid-market.',
    icon: (
      <svg className="w-8 h-8 text-oldgold" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
      </svg>
    )
  }
];

export const Capabilities: React.FC<CapabilitiesProps> = ({ onEquityClick, onCreditClick, onCapOrigClick, onDealOrigClick }) => {
  return (
    <section id="capabilities" className="py-32 bg-obsidian border-b border-white/5 relative">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="mb-24">
          <h2 className="font-display font-medium text-4xl md:text-5xl text-platinum mb-6 tracking-wide">
            Strategic Capabilities
          </h2>
          <p className="font-sans text-gray-400 text-xs uppercase tracking-[0.25em] font-bold">
            Areas of Expertise & Principal Focus
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 border border-white/10">
          {capabilities.map((item, idx) => {
            // Determine handling based on index: 0=Capital Origination, 1=Credit, 2=Deal Origination
            const handleClick = idx === 0 ? onCapOrigClick : idx === 1 ? onCreditClick : onDealOrigClick;

            return (
              <div
                key={idx}
                onClick={handleClick}
                className={`p-12 md:p-16 flex flex-col min-h-[420px] transition-all duration-500 hover:bg-white/[0.02] border-white/10 cursor-pointer group ${idx !== capabilities.length - 1 ? 'lg:border-r border-b lg:border-b-0' : 'border-b lg:border-b-0'
                  }`}
              >
                <div className="mb-12 opacity-80 group-hover:opacity-100 transition-opacity">
                  {item.icon}
                </div>

                <div className="flex-grow">
                  <div className="min-h-[8rem]">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="font-display text-platinum text-2xl tracking-[0.1em] uppercase group-hover:text-oldgold transition-colors">
                        {item.title}
                      </h3>
                      <span className="opacity-0 group-hover:opacity-100 transition-all transform group-hover:translate-x-1 text-oldgold">
                        &rarr;
                      </span>
                    </div>
                    {item.subtitle && (
                      <p className="font-sans text-oldgold text-[10px] uppercase tracking-[0.2em] font-bold mb-8">
                        {item.subtitle}
                      </p>
                    )}                  </div>
                  <div className="w-8 h-[1px] bg-white/20 mb-8 group-hover:bg-oldgold/50 transition-colors"></div>
                  <p className="font-sans text-gray-300 text-lg leading-loose font-light">
                    {item.description}
                  </p>
                </div>
              </div>
            );
          })}
        </div>
      </div>
    </section>
  );
};