...
The old model assumed that sovereign risk premia were static. That duration curves extended predictably. That policy stances could be interpreted as slow-moving signals. These assumptions no longer price correctly. Jurisdictions now compete for credit inflows with the same aggressiveness that firms compete for market share.
...
Capital is an assignment. Waste is rebellion. Stewardship is not charity. It is precision.

Every mandate we accept must meet the filter of non-wasteful deployment. Hard assets that degrade sovereignty neutrality are rejected. Companies that rely on regulatory subsidies are excluded. Structures that depend on political leniency are eliminated.
